...
def make_science_figure(signal, title, extra_info, scenario_code):
    ...
    ax_time=fig.add_subplot(gs[0,:2]); ax_fft=fig.add_subplot(gs[1,:2])
    ax_spec=fig.add_subplot(gs[2,:2]); ax_info=fig.add_subplot(gs[:,2])
    ...
    ax_time.set_xlim(0,t[-1]); ax_time.set_ylim(-1.1,1.1)
    ax_time.set_xlabel("Zeit [s]"); ax_time.set_ylabel("Amplitude")
    ...
    ax_fft.set_xscale("log"); ax_fft.set_xlim(max(freqs[valid][0],1),sr/2)
    db_floor=max(float(np.min(fft_db[valid]))-5,-120)
    ax_fft.set_ylim(bottom=db_floor)
    ax_fft.set_xlabel("Frequenz [Hz]"); ax_fft.set_ylabel("Magnitude [dBFS]")
    ...
    try:
        ...
        ax_spec.set_xlabel("Zeit [s]"); ax_spec.set_ylabel("Frequenz [Hz]")
        ax_spec.set_yscale("log"); ax_spec.set_ylim(20,sr/2)
    except Exception:
...
if do_run:
    sr=SAMPLE_RATE
    if selected_code=="V2a":
        d,fs,fep,amp=params["duration"],params["f_start"],params["f_end_pct"],params["amplitude"]
        t=np.linspace(0,d,int(sr*d))
        f_end=sr/2*(fep/100); L=d/np.log(f_end/fs)
        phase=2*np.pi*fs*L*(np.exp(t/L)-1)
        sig=amp*np.sin(phase)
        extra={"Typ":"Log. Sweep","f_start":f"{fs:.1f} Hz","f_end":f"{f_end:.0f} Hz","Amplitude":f"{amp:.2f}"}
    elif selected_code=="V2b":
        freq,d,env_type=params["freq"],params["duration"],params["env_type"]
        t=np.linspace(0,d,int(sr*d))
        env=(np.linspace(1.0,0.0,len(t)) if env_type=="Linear fallend" else
             np.linspace(0.0,1.0,len(t)) if env_type=="Linear steigend" else
             np.ones(len(t)) if env_type=="Konstant" else
        ...
        sb=int(sr*bm/1000); sp=int(sr*ps/1000)
...
with col_data:
    if "full_signal" not in st.session_state:
        ...
        fs=st.session_state["full_signal"]; ei=st.session_state["extra_info"]
        st_=st.session_state["scenario_title"]; sc=st.session_state["selected_code"]; ts=st.session_state["ts"]
        ...
        fig2=make_science_figure(fs,st_,ei,sc); png_bytes=fig_to_png(fig2); plt.close(fig2)
