...
# --------------------------------------------------
# Matplotlib Style – matches dark UI
# --------------------------------------------------
PLOT_STYLE = {
...
# --------------------------------------------------
# Scientific metrics helper
# --------------------------------------------------
def compute_metrics(signal, sr=SAMPLE_RATE):
    rms   = float(np.sqrt(np.mean(signal**2)))
    peak  = float(np.max(np.abs(signal)))
    crest = float(peak / rms) if rms > 0 else float('inf')
...
# --------------------------------------------------
def make_science_figure(signal, title, extra_info: dict, scenario_code: str):
    ...
    # Spectrogram (short signal → fewer segments)
    seg = min(2048, n // 4) if n > 512 else n
    overlap = seg // 2
    ...
    ax_time   = fig.add_subplot(gs[0, :2])   # waveform
    ax_fft    = fig.add_subplot(gs[1, :2])   # spectrum
    ax_spec   = fig.add_subplot(gs[2, :2])   # spectrogram
    ...
    ax_fft.plot(freqs, fft_db, color=ORANGE, lw=0.7)
    ax_fft.set_xscale("log")
    ax_fft.set_xlim(max(freqs[1], 1), sr / 2)
    ax_fft.set_ylim(bottom=max(fft_db.min() - 5, -120))
    ax_fft.set_xlabel("Frequenz [Hz]")
    ax_fft.set_ylabel("Magnitude [dBFS]")
    ax_fft.fill_between(freqs, fft_db, fft_db.min() - 5, color=ORANGE, alpha=0.06)
    ...
    info_lines = [
        ...
        ("Peak",         f"{metrics['db_peak']:.1f} dBFS"),
        ("RMS",          f"{metrics['rms']:.4f}"),
        ("RMS",          f"{metrics['db_rms']:.1f} dBFS"),
        ("Crest Factor", f"{metrics['crest_db']:.1f} dB"),
        ("",             ""),
    ]

    # Scenario-specific extra
...
with st.sidebar:
    ...
    <div style='font-family:"IBM Plex Mono",monospace; font-size:1rem;
                color:#e6edf3; font-weight:600; margin-bottom:1.5rem;'>
    Messszenarien
    </div>
    """, unsafe_allow_html=True)
...
<div class="nvh-header">
  ...
  <div style="text-align:right">
    ...
    <div class="nvh-badge" style="margin-top:0.4rem">16-bit PCM · Mono</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with col_params:
    ...
    if selected_code == "V2a":
        ...
        f_start  = st.number_input("Startfrequenz (Hz)", 0.1, 1000.0, 0.5, step=0.5,
                                   format="%.1f")
    ...
    # ── V2b: Pegel-Stufentest ──────────────────────
        ...
        env_type = st.selectbox("Hüllkurve", ["Linear fallend", "Linear steigend",
                                               "Konstant", "Sinus-moduliert"])
    ...
    # ── V3: Clipping / Noise Burst ─────────────────
        ...
        window_hann     = np.hanning(samples_burst)
        single_pulse    = amplitude * base_noise * window_hann
...
with col_data:
    if full_signal is not None:
        ...
        fig = make_science_figure(full_signal, scenario_title,
                                  extra_info, selected_code)
        st.pyplot(fig, use_container_width=True)
        plt.close(fig)

        # ── Export as PNG ─────────────────────────
        st.markdown("<div style='height:0.4rem'></div>", unsafe_allow_html=True)
        fig_export = make_science_figure(full_signal, scenario_title,
                                         extra_info, selected_code)
        ...
        if do_run:
            st.markdown('<div class="section-title" style="margin-top:1rem">'
                        'Audioausgabe</div>', unsafe_allow_html=True)
            if selected_code == "V3":
                st.markdown('<div class="warn-banner">'
                            '⚠  Sehr laut — Kopfhörer-Lautstärke reduzieren!</div>',
                            unsafe_allow_html=True)
            wav_bytes = generate_wav_bytes(full_signal)
            st.audio(wav_bytes, format="audio/wav")

            # WAV Download
            wav_fname = f"NVH_{selected_code}_{ts}.wav"
            st.download_button(
...
with st.container():
    ...
    with p2:
        signal_loss = st.number_input(
            "Signalverlust (s)", 0.0, 120.0, 0.0, step=0.1
        )
    with p3:
    ...
    with c1:
        if st.button("Protokoll speichern", use_container_width=True):
            if system != "— auswählen —":
                ...
                st.success(
                    f"✓ Protokolleintrag gespeichert  ·  {ts_now}  ·  "
...
<div style='text-align:center; margin-top:3rem; padding-top:1.5rem;
    ...
    ·  Anthropic Claude  ·  {datetime.datetime.now().strftime('%Y')}
</div>
""", unsafe_allow_html=True)
